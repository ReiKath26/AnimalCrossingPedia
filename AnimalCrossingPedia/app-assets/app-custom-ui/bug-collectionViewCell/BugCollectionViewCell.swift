...
class BugCollectionViewCell: UICollectionViewCell {
    ...
    static let identifier = "BugCollectionViewCell"
    
    var bug: Bugs?
    {
        didSet
        {
            myLabel.text = bug?.name.USName
            
            guard let url = URL(string: bug?.iconURI ?? "http://acnhapi.com/v1/icons/bugs/" + String(bug?.id ?? 1)) else {return}
