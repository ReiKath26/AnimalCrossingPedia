...
class VillagerCollectionViewCell: UICollectionViewCell {
    ...
    static let identifier = "VillagerCollectionViewCell"
    
    var villager: Villager?
    {
        didSet
        {
            myLabel.text = villager?.name.USName
            
            guard let url = URL(string: villager?.iconURL ?? "http://acnhapi.com/v1/icons/villagers/" + String(villager?.id ?? 1)) else {return}
